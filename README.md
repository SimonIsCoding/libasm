...
Built and tested on **macOS (Apple Silicon via Rosetta x86-64)** and **Linux (Ubuntu)**. Not tested on Windows.
...
```
libasm/
├── asm_header.inc    # Cross-platform macros (SYM, CALL, syscall numbers)
├── ft_strlen.s
├── ft_strcpy.s
├── ft_strcmp.s
├── ft_write.s
